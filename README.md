...
A deployment can also theme a run of days from a word list of its own, and give the cryptogram passages of its own — see [docs/selfhost.md](docs/selfhost.md). On a themed day the daily word, the scramble rack and the hive's seven letters can come from that list rather than from the language, boards accept its words even when no dictionary carries them, and each one found is worth +5 on top of the scoring described above; the cryptogram enciphers a passage somebody wrote instead of a curated quotation, per difficulty, where its length suits that difficulty's band. Nothing changes on an ordinary day or in practice.
