...
- **You choose what's remembered** — a first visit asks two separate questions, because they aren't the same question: whether the site may keep your games on this device, and whether analytics may leave it. Storage has three answers — **Remember** (boards, settings, statistics, as before), **Signed in only** (the session token and nothing else; with an account the dailies still follow you, since they live there), and **Forget everything** (nothing written at all — every game still plays, and closing the tab starts over). Changeable later under Settings → Privacy, and choosing less clears what was already there. Everything in the app reads and writes through one gate, so the setting is enforced in a single place rather than in every game
