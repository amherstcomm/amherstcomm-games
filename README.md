...
- **You choose what's remembered** — a first visit asks two separate questions, because they aren't the same question: what the site may keep, and whether analytics may leave the device. Storage has three cumulative answers, ordered by how far the data travels rather than by how much of it there is — **Essential only** (your privacy answers and nothing else; every game still plays, and closing the tab starts over), **Allow browser game data** (boards, settings and statistics kept in this browser, sent nowhere), and **Allow server game data (sign in)** (the above, plus staying signed in, which is what lets results follow you to another device). Sign-in is the top rung rather than the middle one because it's the only rung where anything reaches us. Changeable later under Settings → Privacy; choosing less clears what was already there and signs you out. Everything in the app reads and writes through one gate, so the setting is enforced in a single place rather than in every game
