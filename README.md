...
Ten word games, a fresh puzzle every morning, and a guided demo for each: guess a hidden word, descramble a rack, work a Spelling Bee hive, chain a Letter Boxed square, trace a Strands-style board, fill a word square, break a substitution cipher, climb a word ladder, or find the word that joins SNOW to ROOM.

This is a fork for an internal, staff-only deployment. Upstream ships a solver
alongside each game — type in a board and it answers it — and **this fork does
not**: a solver on a site with prizes on it is a way to win without playing.
The solving *engines* remain, because the games and the Learn demos use them to
mark a board and to show what you missed.
...
Each is a board you play, with a Learn demo beside it. Upstream also ships a
solver for each — a board you type *into* and it answers — and this fork does
not, so this list describes the games rather than the solvers it came with.

- **Guess the Word** (Wordle-style) — a hidden word, six tries, word lengths 3 to 12
- **Scramble** (Jumble-style) — a three-minute sprint through one seven-letter rack
- **Hive** (Spelling Bee-style) — seven letters, centre letter required, up to Queen Bee
- **Grid** (Boggle-style) — trace words through adjacent cells of a 3×3, 4×4 or 5×5
- **Boxed** (Letter Boxed-style) — chain words around twelve letters, no two consecutive letters on one side
- **Squares** — fill a 4×4 or 5×5 so every row *and* column spells a word
- **Weave** (Strands-style) — a themed board, every letter used exactly once
- **Word Ladder** — two words of a length, one letter at a time, every rung a word
- **Bridge** — the word that ends one compound and starts the next: SNOW ▢ ROOM
- **Cryptogram** — a substitution cipher, one passage a day

### Playing, and learning

Every game has a **Play / Learn** switch. Upstream's third position, Solve, is
gone here.
...
One published, versioned artifact ([src/wordbands/](src/wordbands/)), built by `npm run build-words` from SCOWL and nothing else — via wordlist-english up to size 70, and SCOWL's own "huge" (80) list, vendored in [scripts/scowl/](scripts/scowl/), for the top tier: six exclusive bands that every pool is a union of. Generation is one band, acceptance is the bands up to a cut (55 / 70 / 80), and every tier is a size SCOWL itself defined. The same build seeds the Postgres `words` table, so the client and the database cannot disagree by construction. In production the bands come from jsDelivr at a pinned tag with the bundled copies as fallback; **Easy / Hard / Extreme** are exactly the difficulties' accept tiers, so what a board marks as a word is what scores.

A deployment can also theme a run of days from a word list of its own, and give the cryptogram passages of its own — see [docs/selfhost.md](docs/selfhost.md). On a themed day the daily word, the scramble rack, the hive's seven letters, the twelve letters of the letter box and both ends of the word ladder can come from that list rather than from the language, boards accept its words even when no dictionary carries them, and each one found is worth +5 on top of the scoring described above; the cryptogram enciphers a passage somebody wrote instead of a curated quotation, per difficulty, where its length suits that difficulty's band. Nothing changes on an ordinary day or in practice.
...
- **Real addresses** — every state has a URL and the address bar follows you: `/play/hive`, `/daily/squares`, `/learn/grid`, `/stats/history`, `/settings/games`, `/legal/privacy`, `/report/<ticket>`. Opening a panel puts it in the bar and Back closes it; closing one steps back rather than stacking a new entry. Older spellings still work and rewrite themselves on arrival — the `?daily=hive` query links, `/solve/pattern` from when Guess was called Pattern, and every `/solve/<game>` address, which now lands on the board because there is no solver to land on.
