...
- **Weave** (Strands-style, play-only): themed words tile the whole board — every letter used exactly once, with a spangram spanning the board. Drag to trace; theme words lock blue, the spangram gold, and other dictionary words (4+ letters) bank toward hints that outline an unfound word. Puzzles are generated from a curated theme file with a subset-sum + backtracking packer. **Daily** is a 6×8 board; **Practice** draws from a daily pool in 6×8 or 8×10 (hard).
